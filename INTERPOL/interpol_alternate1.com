...
#####################
set met_list1 =  (`awk '{if ($2 == '$Teff_min') print $4}' ${model_path}/grid.list | sort -u`)
set met_list2 = ()
foreach metteffmin ($met_list1)
set met_list2 =  (`awk '{if ($2 == '$Teff_max' && $4 == '$metteffmin') print $4}' ${model_path}/grid.list | sort -u` $met_list2)
end
set met_min = (` echo $met_list2 | awk 'BEGIN{RS=" "}{if (('${zref}' - $1) >=0) print ('${zref}' - $1),$1}' | sort -u | sort -n | awk '{if (NR == 1) print $2}' `)
if ($#met_min == 0) then
set met_min = (`  echo $met_list2 | awk 'BEGIN{RS=" "}{if (('${zref}' - $1) <0) print ('${zref}' - $1)**2,$1}' | sort -u | sort -n | awk '{if (NR == 1) print $2}' `)
set met_max = (`  echo $met_list2 | awk 'BEGIN{RS=" "}{if (('${zref}' - $1) <0) print ('${zref}' - $1)**2,$1}' | sort -u | sort -n | awk '{if (NR == 2) print $2}' `)
else
set met_max = (`  echo $met_list2 | awk 'BEGIN{RS=" "}{if (('${zref}' - $1) <0) print ($1 - '${zref}'),$1}' | sort -u | sort -n | awk '{if (NR == 1) print $2}'`)
if ($#met_max == 0) then
set met_min = (`  echo $met_list2 | awk 'BEGIN{RS=" "}{if (('${zref}' - $1) >= 0) print ($1 - '${zref}')**2,$1}' | sort -u | sort -n | awk '{if (NR == 2) print $2}'`)
set met_max = (` echo $met_list2 | awk 'BEGIN{RS=" "}{if (('${zref}' - $1) >= 0) print ($1 - '${zref}')**2,$1}' | sort -u | sort -n | awk '{if (NR == 1) print $2}'`)
endif
endif
